...
var arr = new Array();*/
var source,target,test,start,q,flagIn=false;
var edge;
var stepDelay = 900;
var data={"nodes":[],"links":[],"attrs":[]}
var neighbor=[],d=[],v=[],f=[], qExist=[],currentNaighbor=[];
...
this.initArrays=function(val){
  ...
  // q.enqueue(Number(start));
   for(var i=0;i<sizeArray;i++)
   {
    d[i]=0;
    f[i]=0;   
    data["attrs"].push({"node":this.storage[i],"color":"White","d":d[i],"f":f[i]});
    v[i]="NIL";
   ...
   data.attrs[0]["v"]="NIL";
    ...
    for(var i=0;i<sizeArray;i++)
    {
        if(data.attrs[i]["node"]==Number(start)){
            flagIn=true;
            indexStart=i;
            if(data.attrs[i]["color"]=="White"){
               Dfs_Visit(data.attrs[i]["node"],i)
               //upDateRows(v,d,f);
            }
...
function GetNaighbors(val){
    for(var i=0;i<data.links.length;i++)
    {
        if(data.links[i]["source"]==val)
        {
           neighborU.enqueue(data.links[i]["target"]);
        }
        if(data.links[i]["target"]==val)
        {
           neighborU.enqueue(data.links[i]["source"]);
        }
    }
}
function Dfs_Visit(u,index){
    //UpdateDFStables();
   // upDateRows(v,d,f);
    time=time+1;
    var v1;
    d[index]=time;
    data.attrs[index]["d"]=time;
    data.attrs[index]["color"]="Gray";
    GetNaighbors(u);
    while(!neighborU.isEmpty())
    {
        v1=neighborU.dequeue();
        if(data.attrs[GetIndexOfNode(v1)]["color"]=="White")
        {
            v[GetIndexOfNode(v1)]=u;
            data.attrs[GetIndexOfNode(v1)]["v"]=u;
            Dfs_Visit(v1,GetIndexOfNode(v1));
        }
    }
    time=time+1;
    f[index]=time;
    data.attrs[index]["f"]=time;
    data.attrs[index]["color"]="black";
    upDateRows(data.attrs[index]["v"],data.attrs[index]["d"],data.attrs[index]["f"]);
   ...
   for(var i=0;i<sizeArray-1;i++)
   {
    if(data.links[i]["source"] ==Number(start))
    {
        if(IsContains(qExist,data.links[i]["target"])==false)
        {
          ...
          data.attrs[GetIndexOfNode(data.links[i]["target"])]["color"]="Gray";
    ...
    if(data.links[i]["target"] ==Number(start))
    {
        if(IsContains(qExist,data.links[i]["source"])==false)
        {
          ...
          data.attrs[GetIndexOfNode(data.links[i]["source"])]["color"]="Gray";
...
DFS.prototype.removeMax = function() {
