...
//
 ...
 //  row=tbody.append('tr').attr("class", "valColumn");
  ...
  // .attr('class','td-val')
  //.data(hp.storage)
  //// .data(hp.storage.sort(function(a, b){return a-b}))
  ////  .sort(d3.ascending)
  // .enter()
  // .append("td")
...
var insertNode = function(value) {
  ...
  link = link.data(nodes.slice(1), function(d) {
...
return "M" + d.x + "," + d.y +
   ...
   // return diagonal({source: o, target: o});
 // });
// link.data((tree.nodes(root)).slice(1)).enter().insert("path", ".node")
// .attr("class", "link")
// .attr("d", function(d) {
//   var o = {x: d.source.px, y: d.source.py};
//   return diagonal({source: o, target: o});
...
var animateSwap = function() {
  ...
  link = link.data(nodes, function(d) 
  { 
      return d;
//   if (d.parent && d.parent.children.length == 1) {
//       if (d.parent.parent)
...
var algorithim=["Add a root for the tree","Add Child At Left","Add Child at Right","If Parent value Bigger than current-SWITCH"];
...
function DeleteAlertAlgo(ind){   
    d3.select("#algo").selectAll("div").remove();
    var algorithim=["delete root","Add Child At Left","Add Child at Right","If Parent value Bigger than current-SWITCH"];
